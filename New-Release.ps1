...
if( !(Test-Path 'env:tools') ) {
  $env:tools = 'C:\tools\'
  Write-Host "|  - defaulting to '$env:tools'"
}

$rar = Test-Tool 'WinRar' "$env:tools\programs\WinRar\rar.exe"

$vs = Test-Tool 'VS2015' (Join-Path $env:VS140COMNTOOLS '..\IDE\devenv.exe')

if( -not $xunitPath ) { $xunitPath = '.\packages\xunit.runner.console.2.3.0\tools\net452\xunit.console.exe' }
...
foreach( $release in $releases ) {
  ...
  $releaseVersionParts = @(0,1)
  if( $assemblyVersion[3] ) { $releaseVersionParts += 3 }
  $releaseVersion = [string]::Join( '.', $assemblyVersion[$releaseVersionParts] )
  ...
  if( $release -eq 'Release' ) {
    ...
    foreach( $ignorePath in @('.tfignore','.hgignore') ) {
      if( Test-Path $ignorePath ) {
        $ignoredFiles += @(Get-Content $ignorePath | where { $_ -and $_[0] -ne '#' })
      }
    }
    ...
    $sourceFiles = Get-ChildItem $projectRoot -recurse | ? {
      ...
      $fullname = $_.FullName
      -not ($exports,'.hg','bin','obj','packages','TestResults' | ? { $fullname -match ".*[\\/]$_.*" })
    } | ? {
      $name = $_.Name
