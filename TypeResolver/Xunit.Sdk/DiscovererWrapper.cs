    ...
    public abstract class DiscovererWrapper : IXunitTestCaseDiscoverer {
        ...
        public IEnumerable<IXunitTestCase> Discover( ITestFrameworkDiscoveryOptions discoveryOptions, ITestMethod testMethod, IAttributeInfo factAttribute ) {
            ...
            try {
                ...
                string errorMessage = "Failure processing test method " + testMethod.Method.ToRuntimeMethod( ).GetDescriptiveName( );
                if( ex.Data.Contains( TypeResolver.TypeLoader.TypeLoaderAssemblyKey ) )
                    errorMessage += " while loading assembly " + ex.Data[TypeResolver.TypeLoader.TypeLoaderAssemblyKey];
                errorMessage += ":";

                var failureInfo = ExceptionUtility.ConvertExceptionToFailureInformation( ex );
                for( int i = 0; i < failureInfo.ExceptionTypes.Length; ++i ) {
                    int level = 1 + failureInfo.ExceptionParentIndices[i];
                    string type = failureInfo.ExceptionTypes[i];
                    string message = failureInfo.Messages[i];
                    string stack = failureInfo.StackTraces[i];
                    errorMessage += string.Join( Environment.NewLine, "", level, type, message, stack );
                }

                return new ExecutionErrorTestCase( this.diagnosticMessageSink_, defaultMethodDisplay, testMethod, errorMessage )
                    .MakeEnumerable<IXunitTestCase>( );
            }
        }
